...
async function requestJson(url, options) {
  ...
  if (!response.ok) throw new Error(`Wan API ${response.status}: ${body.message || body.code || response.statusText}`);
  return body;
}
...
export async function submitWan3Shot({job, shot}) {
  ...
  const media = [...characterMedia, ...(shot.media || [])].filter((item) => item?.type && item?.url && !item.url.includes('example.invalid'));
  ...
  return {taskId, baseUrl, apiKey, requestId: body.request_id};
}

export async function pollWan3Task({taskId, baseUrl, apiKey, intervalMs = 15000, maxPolls = 80}) {
  for (let attempt = 0; attempt < maxPolls; attempt += 1) {
    const body = await requestJson(`${baseUrl}/tasks/${taskId}`, {headers: {Authorization: `Bearer ${apiKey}`}});
    ...
    if (['FAILED','CANCELED','UNKNOWN'].includes(status)) throw new Error(`Wan task ${taskId} ended with status ${status}`);
...
async function main() {
  ...
  if (shot.status !== 'APPROVED') throw new Error(`Shot ${shotId} is ${shot.status || 'UNSPECIFIED'}; paid generation requires status APPROVED`);
